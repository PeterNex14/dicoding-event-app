...
class BookmarkedViewModel(private val repository: EventRepository) : ViewModel() {
    private var _syncStatus = MutableLiveData<Result<Unit>>()
    val syncStatus: LiveData<Result<Unit>> = _syncStatus

    val bookmarkedEvents = repository.getBookmarkedEvents()
